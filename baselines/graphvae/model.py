...
class GraphVAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, max_num_nodes, pool='sum', is_cuda_available=False):
        """
        Args:
        ...
        self.conv1 = model.GraphConv(input_dim=input_dim, output_dim=hidden_dim, is_cuda_available=self.is_cuda_available)
        self.bn1 = nn.BatchNorm1d(input_dim)
        self.conv2 = model.GraphConv(input_dim=hidden_dim, output_dim=hidden_dim, is_cuda_available=self.is_cuda_available)
        self.bn2 = nn.BatchNorm1d(input_dim)
        self.act = nn.ReLU()

        output_dim = max_num_nodes * (max_num_nodes + 1) // 2
        # self.vae = model.MLP_VAE_plain(hidden_dim, latent_dim, output_dim)
        self.vae = model.MLPVAEPlain(input_dim * input_dim, latent_dim, output_dim, is_cuda_available=self.is_cuda_available)
    ...
    def forward_test(self, input_features, adj):
        ...
        S = self.edge_similarity_matrix(adj_data, adj_data1, adj_features, adj_features1, self.deg_feature_similarity)
