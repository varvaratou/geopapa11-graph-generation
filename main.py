...
if __name__ == '__main__':
    ...
    if 'GraphRNN_VAE_conditional' in args.note:
        ...
        output = MLP_VAE_conditional_plain(h_size=args.hidden_size_rnn,
                                           embedding_size=args.embedding_size_output, y_size=args.max_prev_node)  # .cuda()
    elif 'GraphRNN_MLP' in args.note:  # GraphRNN-S (theta_i \in R^{i-1}: theta_i[j]: probability of edge (i, j))
        rnn = GRU_plain(input_size=args.max_prev_node, embedding_size=args.embedding_size_rnn,
                        hidden_size=args.hidden_size_rnn, num_layers=args.num_layers, has_input=True,
                        has_output=False)  # .cuda()
    ...
    #           graph_validate_len=graph_validate_len, graph_test_len=graph_test_len)
