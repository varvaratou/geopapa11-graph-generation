...
class Args_evaluate:
    def __init__(self):
        ...
        # self.dataset_name_all = ['barabasi_noise0','barabasi_noise2','barabasi_noise4','barabasi_noise6',
        #                          'barabasi_noise8','barabasi_noise10']
        # self.dataset_name_all = ['caveman_small', 'ladder_small', 'grid_small', 'ladder_small', 'enzymes_small',
        #                          'barabasi_small','citeseer_small']
...
def evaluation(args_evaluate, dir_input, dir_output, model_name_all, dataset_name_all, args, overwrite=True):
    ...
    for model_name in model_name_all:
        for dataset_name in dataset_name_all:
            ...
            if not overwrite and os.path.isfile(fname_output):
            ...
            evaluation_epoch(dir_input, fname_output, model_name, dataset_name, args,
                             is_clean=True, epoch_start=args_evaluate.epoch_start,
    ...
    for i in range(num_evals):
        ...
        if i == 0:
            ...
            for baseline in baselines:
                ...
                print('Kron: deg=', dist_degree, ', clustering=', dist_clustering, ', orbits4=', dist_4orbits)
...
def eval_performance(datadir, prefix=None, args=None, eval_every=200, out_file_prefix=None, sample_time=2, baselines={}):
    if args is None:
        real_graphs_filename = [datadir + f for f in os.listdir(datadir) if re.match(prefix + '.*real.*\.dat', f)]
        pred_graphs_filename = [datadir + f for f in os.listdir(datadir) if re.match(prefix + '.*pred.*\.dat', f)]
