...
def train_vae_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        if epoch % args.epochs_log == 0 and batch_idx == 0:  # only output first batch's statistics
            print(f'[{strftime("%Y-%m-%d %H:%M:%S", localtime())}] Epoch: {epoch}/{args.epochs}: '
...
def test_vae_partial_epoch(epoch, args, rnn, output, data_loader, save_histogram=False, sample_time=1):
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        for i in range(max_num_node):
            ...
            y_pred_step, _, _ = output(h)
            y_pred[:, i:i + 1, :] = F.sigmoid(y_pred_step)
            x_step = sample_sigmoid_supervised(y_pred_step, y[:, i:i+1, :],  # .cuda(),
                                               current=i, y_len=y_len, sample_time=sample_time)
...
def train_mlp_epoch(epoch, args, rnn, output, data_loader, optimizer_rnn, optimizer_output, scheduler_rnn, scheduler_output):
    rnn.train()
    output.train()
    loss_sum = 0
    for batch_idx, data in enumerate(data_loader):
...
def test_mlp_partial_simple_epoch(epoch, args, rnn, output, data_loader, save_histogram=False, sample_time=1):
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        for i in range(max_num_node):
            ...
            y_pred_step = output(h)
            y_pred[:, i:i + 1, :] = F.sigmoid(y_pred_step)
            x_step = sample_sigmoid_supervised_simple(y_pred_step, y[:, i:i+1, :],  # .cuda(),
                                                      current=i, y_len=y_len, sample_time=sample_time)
...
def train_rnn_epoch(epoch, args, rnn, output, data_loader, optimizer_rnn, optimizer_output, scheduler_rnn, scheduler_output):
    rnn.train()
    output.train()
    loss_sum = 0
    for batch_idx, data in enumerate(data_loader):
...
def train(args, dataset_train, rnn, output):
    ...
    if args.load:
        fname = os.path.join(args.model_save_path, args.fname + str(args.load_epoch) + '.dat')
        rnn.load_state_dict(torch.load(fname))
        fname = os.path.join(args.model_save_path, args.fname + 'output_' + str(args.load_epoch) + '.dat')
    ...
    while epoch <= args.epochs:
        ...
        if 'GraphRNN_VAE' in args.note:
            train_vae_epoch(epoch, args, rnn, output, dataset_train, optimizer_rnn, optimizer_output, scheduler_rnn, scheduler_output)
        elif 'GraphRNN_MLP' in args.note:
            train_mlp_epoch(epoch, args, rnn, output, dataset_train, optimizer_rnn, optimizer_output, scheduler_rnn, scheduler_output)
        elif 'GraphRNN_RNN' in args.note:
            train_rnn_epoch(epoch, args, rnn, output, dataset_train, optimizer_rnn, optimizer_output, scheduler_rnn, scheduler_output)
        time_end = tm.time()
                ...
                while len(G_pred) < args.test_total_size:
                    if 'GraphRNN_VAE' in args.note:
                        G_pred_step = test_vae_epoch(epoch, args, rnn, output,
                                                     test_batch_size=args.test_batch_size, sample_time=sample_time)
                    elif 'GraphRNN_MLP' in args.note:
...
def train_graph_completion(args, dataset_test, rnn, output):
    ...
    for sample_time in range(1, 4):
        ...
        save_graph_list(G_pred, fname)
    print('[{strftime("%Y-%m-%d %H:%M:%S", localtime())}] Graph completion done, graphs saved!')
